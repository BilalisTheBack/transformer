    ...
    <div className="max-w-6xl mx-auto h-[calc(100vh-8rem)] flex flex-col gap-6 animate-in fade-in duration-500 pb-24">
      <header className="space-y-2">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-3">
          ...
          {t("css.gradient.title")}
        </h1>
        <p className="text-gray-600 dark:text-gray-400">
          {t("css.gradient.description")}
        </p>
      </header>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 shadow-sm space-y-4">
            <div>
              ...
              <div className="flex gap-2">
                {(["linear", "radial", "conic"] as const).map((typeOption) => (
                  <button
                    key={typeOption}
            ...
            {(type === "linear" || type === "conic") && (
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  {type === "linear"
                    ? t("css.gradient.angle")
                    : t("css.gradient.startAngle")}
                  : {angle}°
                </label>
                <input
                  type="range"
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 shadow-sm">
            <div className="flex justify-between items-center mb-3">
              <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">
                {t("css.gradient.colorStops")}
              </h3>
              <button
                onClick={addStop}
                className="text-sm text-purple-600 dark:text-purple-400 hover:underline flex items-center gap-1"
              >
                <Plus className="w-4 h-4" /> {t("css.gradient.add")}
              </button>
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 shadow-sm">
            <div className="flex justify-between items-center mb-2">
              ...
              >
                ...
                {copied ? t("css.gradient.copied") : t("css.gradient.copy")}
              </button>
            </div>
